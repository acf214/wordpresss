...
<header id="header">	
	...
	<?php //var_dump ( $chronicle_theme_options['title_font']); ?>
		<a href="<?php echo esc_url(home_url( '/' )); ?>" style="color:#<?php if(header_textcolor()){ echo esc_attr(header_textcolor());} ?>" id="sitename" title="<?php echo esc_attr( get_bloginfo( 'name', 'display' ) ); ?>">
		<?php 
		$custom_logo_id = get_theme_mod( 'custom_logo' );
		$image = wp_get_attachment_image_src( $custom_logo_id,'full' );
		if(has_custom_logo())	{ ?>
		<img class="img-responsive site-custom_logo" src="<?php echo esc_attr($image[0]); ?>" style="height:<?php if($chronicle_theme_options['height']!='') { echo esc_attr($chronicle_theme_options['height']); } ?>px; width:<?php if($chronicle_theme_options['width']!='') { echo esc_attr($chronicle_theme_options['width']); } ?>px;" /><?php 
	...
	<div class="right_links chronicle_social_header">
        <ul> <?php if($chronicle_theme_options['contact_email']!=''){ ?>           	
			<li class="link chronicle_social_email"><a href="mailto:<?php echo esc_url($chronicle_theme_options['contact_email']); ?>"><i class="fa fa-envelope"></i> <?php echo esc_attr($chronicle_theme_options['contact_email']); ?></a></li>
			<?php } if($chronicle_theme_options['phone_no']!=''){ ?> 
			<li class="link chronicle_social_phone"><i class="fa fa-phone"></i> + <?php echo esc_attr($chronicle_theme_options['phone_no']); ?></li>
	...
	<!-- Menu -->
	<div class="menu_main"  <?php if($chronicle_theme_options['sticky_header']=='on') { ?> data-spy="affix" data-offset-top="95" <?php } ?>>    
    <div class="container">        
